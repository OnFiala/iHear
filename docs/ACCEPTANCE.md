...
The current report template is version 4 (Clear Signal); version 2 evidence below is historical. It shows unavailable measurements honestly, displays captured times in the clinic timezone, and begins scheduled preparation at 08:00 one clinic-local calendar day before follow-up. A new result invalidating a requested report produces a visible outdated notice rather than silently offering a stale PDF.
