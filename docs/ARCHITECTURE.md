...
`ListeningEvent`: `{id,patientId,kind,difficulty,environment,capturedAt,createdAt,status:'queued'|'analysing'|'ready'|'failed',capture,profileSnapshot,currentAids?:object,analysis:object|null,interpretation:{status:string,promptVersion:string,model:string,result:object|null}|null,error?:string|null}`.
