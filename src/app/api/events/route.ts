...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof metadataValue !== "string")
      throw new HttpError(400, "Event metadata is required.");
    if (metadataValue.length > 65_536)
      throw new HttpError(413, "Event metadata is too large.");
    if (!(audioValue instanceof Blob))
      throw new HttpError(400, "A WAV recording is required.");
    let parsed: unknown;
    try {
      parsed = JSON.parse(metadataValue);
    } catch {
      throw new HttpError(400, "Event metadata must be valid JSON.");
    }
    const event = await acceptEvent(
      request,
      auth.workspaceId,
      auth.patientId,
