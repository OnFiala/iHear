...
export async function GET(request: NextRequest, context: Context) {
  try {
    ...
    const [patient, events] = await Promise.all([getPatient(auth.workspaceId, id), listEvents(auth.workspaceId, id)]);
...
export async function PATCH(request: NextRequest, context: Context) {
  try {
    ...
    const { id: rawId } = await context.params;
    const id = parseUuid(rawId, 'Patient ID');
    const patient = await updatePatient(auth.workspaceId, id, parseProfile(await readJson(request)));
