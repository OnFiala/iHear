...
type CapabilityRow = Record<string, unknown> & { kind: 'owner' | 'patient'; workspace_id: string; patient_id: string | null };

export type OwnerAuth = { kind: 'owner'; workspaceId: string };
export type PatientAuth = { kind: 'patient'; workspaceId: string; patientId: string; patient: Patient };
export type SessionAuth = OwnerAuth | PatientAuth;

async function resolveCapability(request: NextRequest, cookieName: string, expectedKind: 'owner' | 'patient'): Promise<SessionAuth | null> {
  ...
  const row = rows[0];
  if (!row) return null;
  await query('update ihear.capabilities set last_used_at = now() where token_hash = $1', [hashCapability(token)]);
  if (row.kind === 'owner') return { kind: 'owner', workspaceId: row.workspace_id };
  if (!row.patient_id || !row.id) return null;
  return { kind: 'patient', workspaceId: row.workspace_id, patientId: row.patient_id, patient: patientFromRow(row) };
}

export async function resolveOwnerSession(request: NextRequest): Promise<OwnerAuth | null> {
  return await resolveCapability(request, OWNER_COOKIE, 'owner') as OwnerAuth | null;
}

export async function resolvePatientSession(request: NextRequest): Promise<PatientAuth | null> {
  return await resolveCapability(request, PATIENT_COOKIE, 'patient') as PatientAuth | null;
}

export async function resolveSession(request: NextRequest): Promise<SessionAuth | null> {
  return await resolvePatientSession(request) ?? await resolveOwnerSession(request);
}

export async function createOwnerSession(request: NextRequest): Promise<{ auth: OwnerAuth; token: string }> {
...
export async function requirePatient(request: NextRequest): Promise<PatientAuth> {
