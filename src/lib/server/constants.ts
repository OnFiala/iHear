...
function fixedName(variable: string, expected: string): string {
  ...
  if (!/^[a-z0-9][a-z0-9_-]{2,62}$/.test(value)) throw new Error(`${variable} has an invalid name.`);
  if (value !== expected) throw new Error(`${variable}=${value} does not match the applied database migration (${expected}).`);
  return value;
}
...
export const REPORT_VERSION = fixedVersion('REPORT_VERSION', 1);
