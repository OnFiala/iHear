...
export async function acceptEvent(
  ...
  if (metadata.patientId !== patientId)
    throw new HttpError(403, "The event is outside this patient session.");
  const wav = inspectWav(audio);
  if (wav.sampleRate !== metadata.capture.sampleRate)
    throw new HttpError(
      400,
      "The WAV sample rate does not match the capture metadata.",
    );
  ...
  await query("select ihear.finalize_event_upload($1, $2, $3, $4)", [
    metadata.id,
    workspaceId,
    patientId,
    fingerprint,
