...
export function randomPairingToken(length = 32): string {
  ...
  let result = '';
  for (let index = 0; index < length; index += 1) result += BASE32[bytes[index] % BASE32.length];
  return result;
}
...
function allowedOrigins(request: NextRequest): Set<string> {
  ...
  if (process.env.NODE_ENV !== 'production') {
    for (const value of (process.env.DEV_ALLOWED_ORIGINS ?? request.nextUrl.origin).split(',')) {
...
export function requireSameOrigin(request: NextRequest): void {
  ...
  if (!allowedOrigins(request).has(normalized)) throw new HttpError(403, 'A same-origin request is required.');
}

function rateLimitSalt(): string {
  const salt = process.env.IHEAR_RATE_LIMIT_SALT;
  if (salt) return salt;
  if (process.env.NODE_ENV === 'production') throw new Error('IHEAR_RATE_LIMIT_SALT is not configured.');
  return 'ihear-local-rate-limit-v1';
}

export function requestIpHash(request: NextRequest): Buffer {
  const forwarded = request.headers.get('x-forwarded-for')?.split(',')[0]?.trim();
  const address = (forwarded || request.headers.get('x-real-ip') || 'local').slice(0, 128);
  return createHmac('sha256', rateLimitSalt()).update(`ip:${address}`, 'utf8').digest();
}

export function setSessionCookie(response: NextResponse, request: NextRequest, scope: 'owner' | 'patient', token: string, expires?: Date): void {
  const forwardedProtocol = request.headers.get('x-forwarded-proto')?.split(',')[0]?.trim();
  const secure = process.env.NODE_ENV === 'production' || forwardedProtocol === 'https' || request.nextUrl.protocol === 'https:';
  response.cookies.set(scope === 'owner' ? OWNER_COOKIE : PATIENT_COOKIE, token, {
    httpOnly: true,
    sameSite: 'lax',
    secure,
    path: '/',
